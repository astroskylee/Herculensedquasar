...
def projected_mge_enclosed_flux(R, amp, sigma):
    return np.sum(amp * (1.0 - np.exp(-0.5 * (R / sigma) ** 2)))


def projected_mge_half_light_radius(amp, sigma):
    amp = np.asarray(amp, dtype=float)
    sigma = np.asarray(sigma, dtype=float)
    total_flux = np.sum(amp)
    return brentq(
        lambda R: projected_mge_enclosed_flux(R, amp, sigma) - 0.5 * total_flux,
        0.0,
        20.0 * np.max(sigma),
    )


STELLAR_SCALE_RADIUS_ARCSEC = projected_mge_half_light_radius(
    fixed_five_gaussians['amp'],
    fixed_five_gaussians['sigma'],
)
FIXED_RS_HALO_ARCSEC = 10.0 * STELLAR_SCALE_RADIUS_ARCSEC
print(f'Projected stellar scale radius R_half = {STELLAR_SCALE_RADIUS_ARCSEC:.6f} arcsec')
print(f'Fixed gNFW Rs_halo = 10 * R_half = {FIXED_RS_HALO_ARCSEC:.6f} arcsec')
...
for i in range(num_chains):
    ...
    print(
        ...
        f'Rs_halo_fixed={FIXED_RS_HALO_ARCSEC:.4f}, '
...
mass_cosmo_std_diag_dict = {
    ...
    'theta_E_g1[0]': 0.0036,
    'theta_E_g2[0]': 0.036,
    'omega_m_cosmo': 1,   # temporary fallback
    'H0_cosmo': 5,        # temporary fallback
    'kappa_ext': 0.046,
}
