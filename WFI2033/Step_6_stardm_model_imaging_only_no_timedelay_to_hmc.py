...
STEP3_COSMO_TAG = 'infer_LCDM_no_timedelay'
...
def add_single_time_delay_likelihood(dt_name, like_name, prefactor_days, fpd, obs_key):
    dt = add_single_time_delay_deterministic(dt_name, prefactor_days, fpd)
    numpyro.factor(
        like_name,
        Numpyro_function.split_normal_logpdf(
            dt,
...
def add_single_time_delay_deterministic(dt_name, prefactor_days, fpd):
    return numpyro.deterministic(dt_name, prefactor_days * fpd)
...
def model_step6(data_subtracted, stage_kwargs):
    ...
    if stage_kwargs['compute_fermat_diffs']:
        ...
        if stage_kwargs.get('sample_cosmology', False):
            ...
            if stage_kwargs.get('use_time_delay_likelihood', False):
                ...
                add_single_time_delay_deterministic('dt_31_days', prefactor_days, fpd_31)
                add_single_time_delay_deterministic('dt_32_days', prefactor_days, fpd_32)
                add_single_time_delay_deterministic('dt_34_days', prefactor_days, fpd_34)
