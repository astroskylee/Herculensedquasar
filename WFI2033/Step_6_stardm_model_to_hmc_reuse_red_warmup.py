...
run_tag = '20260427_14'
STEP3_COSMO_PRIOR = 'DESI_PLANCK'  # or 'DESI_PLANCK'
RED_WARMUP_SOURCE_TAG = '20260504_16'# or 0502_06
RED_WARMUP_CHAIN_INDEX = 3  # matplotlib C3, the red chain
...
SOURCE_STEP3_SUFFIX = f"{suffix}_step6_{STEP3_COSMO_TAG}"
STEP3_SUFFIX = f"{SOURCE_STEP3_SUFFIX}_reuse_redwarmup"
...
RED_WARMUP_OUTPUT_DIR = OUTPUT_ROOT / f"WFI2033{SOURCE_STEP3_SUFFIX}_{RED_WARMUP_SOURCE_TAG}"
RED_WARMUP_STATE_PATH = RED_WARMUP_OUTPUT_DIR / 'resume_state_after_warmup.pkl'
...
def select_chain_leaf(value, chain_index, n_chains):
    value = jnp.asarray(value)
    if value.ndim > 0 and value.shape[0] == n_chains:
        return value[chain_index]
    return value


def select_chain_tree(tree, chain_index, n_chains):
    return jax.tree.map(
        lambda value: select_chain_leaf(value, chain_index, n_chains),
        tree,
    )


print('red warmup source state:', RED_WARMUP_STATE_PATH)
red_warmup_state = ResumeInit.load_resume_state(RED_WARMUP_STATE_PATH)
red_step_size = select_chain_leaf(
    red_warmup_state.adapt_state.step_size,
    RED_WARMUP_CHAIN_INDEX,
    num_chains,
)
red_inverse_mass_matrix = select_chain_tree(
    red_warmup_state.adapt_state.inverse_mass_matrix,
    RED_WARMUP_CHAIN_INDEX,
    num_chains,
)

print(f'reusing warmup adaptation from chain index {RED_WARMUP_CHAIN_INDEX}')
print('red step size:', np.asarray(red_step_size))
print('red inverse mass matrix block shapes:')
for block_name, block_value in red_inverse_mass_matrix.items():
    print(f'  {block_name}: {tuple(np.asarray(block_value).shape)}')
...
stage3_kernel = NUTS(
    ...
    step_size=red_step_size,
    inverse_mass_matrix=red_inverse_mass_matrix,
